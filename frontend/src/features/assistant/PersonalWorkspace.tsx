...
export function PersonalWorkspace({ revision }: { revision: number }) {
  ...
  const [kind, setKind] = useState<'memory' | 'note'>('memory');
  ...
  const save = () => perform(async () => {
    ...
    await refreshEntries(); reset('memory'); setMessage('已保存到个人工作区。');
  });
  ...
  return <details className="detail-card assistant-workspace">
    <summary>我的工作区与个人偏好</summary>
    ...
    <div className="toolbar"><button type="button" onClick={() => reset('memory')}>新增 memory</button></div>
    <h3>{editing ? '编辑' : '新建'}{kind === 'note' ? '已保存笔记' : '个人记忆'}</h3>
    ...
    <h3>已保存的记忆与笔记</h3>
    <p className="muted">实验记录请前往<a href="/experiments">实验日志</a>。已有笔记保留，可继续查看、编辑或整理共享版本。</p>
    {entries.map((entry) => <article className="source-preview" key={entry.id}>
      <strong>{entry.kind === 'memory' ? 'Memory' : '已保存笔记'} #{entry.id} · {entry.title}</strong>
      ...
      <button type="button" disabled={busy} onClick={() => perform(async () => { await workspaceRequest(`workspace/entries/${entry.id}/`, 'DELETE'); await refreshEntries(); if (editing?.id === entry.id) reset('memory'); })}>删除</button>
