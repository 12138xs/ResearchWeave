...
def personal_context(user, *, include_memory=True):
    ...
    memories = list(PersonalEntry.objects.filter(owner=user, kind="memory", enabled=True).order_by("pk").values("id", "body")[:20]) if enabled and include_memory else []
