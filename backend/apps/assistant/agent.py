...
for name, description, extra in [
    ...
    ("find_in_source", "在本轮已找到材料或结构化知识文档的固定版本正文内，用正文关键词定位实验、限制或附录等远处证据，返回最多三个片段。不扩展材料范围。", {"query": {"type": "string", "minLength": 1, "maxLength": 500}}),
]:
...
材料版本号只能使用 version_number 或 location 中的版本号；不要把其他编号当作文档修订号。无需向用户解释内部 source_kind 等字段名，直接用中文来源类型。
