...
class ClientTests(unittest.TestCase):
    ...
    @patch("researchweave_mcp.client.urlopen")
    def test_request_uses_rest_bearer_without_logging_or_returning_token(self, opened):
        opened.return_value = Response({"capabilities": ["materials.list"]}, headers={"X-Request-ID": "server-id"})
        config = ClientConfig(base_url="https://a510.example", token="rwv_pat_example.secret")
        result = ResearchWeaveClient(config).get_me()
    ...
    @patch("researchweave_mcp.client.urlopen")
    def test_api_error_is_structured_and_does_not_include_token(self, opened):
        ...
        client = ResearchWeaveClient(ClientConfig(base_url="https://a510.example", token="rwv_pat_example.secret"))
